...
parser.add_argument('--attn_transform', default='constrained_sparsemax',
                    help='Parent dir of the dataset')
    ...
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        ...
        if params.gan == 'False':
            ...
            fake_input = torch.cat(
                (labels_batch[:, :params.predict_start], q50), 1)
            # -------------------------------------------------------------------
            # Train the generator
            # -------------------------------------------------------------------
            optimizer_G.zero_grad()
    ...
    for epoch in range(params.num_epochs):
        ...
        if is_best:
            ...
            best_json_path = os.path.join(
                params.model_dir, 'metrics_test_best_weights.json')
            utils.save_dict_to_json(test_metrics, best_json_path)

        utils.save_loss(loss_summary[epoch * train_len:(epoch + 1) * train_len],
                        args.dataset + '_' + str(epoch) + '-th_epoch_loss', params.plot_dir)
        utils.save_loss(loss_test[epoch * test_len:(epoch + 1) * test_len],
                        args.dataset + '_' + str(epoch) + '-th_epoch_test_loss', params.plot_dir)

        last_json_path = os.path.join(
            params.model_dir, 'metrics_test_last_weights.json')
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_set, batch_size=params.batch_size,
                              sampler=RandomSampler(train_set), num_workers=4)
    ...
    n_updates_total = (train_set.__len__() //
