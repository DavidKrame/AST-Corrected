...
class TrainDataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        self.data = np.load(os.path.join(
            data_path, f'train_data_{data_name}.npy'))
        self.label = np.load(os.path.join(
            data_path, f'train_label_{data_name}.npy'))
...
class ValidDataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        self.data = np.load(os.path.join(
            data_path, f'valid_data_{data_name}.npy'))
        self.v = np.load(os.path.join(data_path, f'valid_v_{data_name}.npy'))
        self.label = np.load(os.path.join(
            data_path, f'valid_label_{data_name}.npy'))
...
class TestDataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        self.data = np.load(os.path.join(
            data_path, f'test_data_{data_name}.npy'))
        self.v = np.load(os.path.join(data_path, f'test_v_{data_name}.npy'))
        self.label = np.load(os.path.join(
            data_path, f'test_label_{data_name}.npy'))
        #self.time = np.load(os.path.join(data_path, f'test_time_{data_name}.npy'))
