...
class Embedding(nn.Module):
    def __init__(self, params, position):
        ...
        '''
        self.embedding
        self.embed1 = nn.Linear(params.embedding_dim +
                                params.cov_dim + 1, params.d_model)
    ...
    def forward(self, x, idx):
        ...
        onehot_embed = self.embedding(
            idx)  # [bs, windows_len, embedding_dim(default 20)]
        try:
...
def test(model, params, x, v_batch, id_batch):
    ...
    sample_mu = torch.zeros(
        batch_size, params.predict_steps, device=params.device)
